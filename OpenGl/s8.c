/* this iteration will be on uniforms. a uniform variable is the opposite of a varying variable. 
 * inother words, they are the same for all vertices. or you might consider them to be to be like
 * global variables. also incontrast to varyings, there is an interface to C for them. and i will
 * show you such now. */
...
GLint uniform_fade;
//yup, speaking of globabls, another one. this will hold the reference to the uniform.
...
int init_resources()
{
	...
	if (uniform_fade == -1)
	{//this whole process should look familiar, works just like attributes, just with the word
	//"uniform" instead of "attrib" or "attribute"
...
void display() 
{
	...
	//glUniform1f. this is used to assign a value to a uniform. and as you can see by its name
	//it takes 1 floating point value (1f). you pass what uniform you are wanting to assign and
	//then the value you want to assign to it.
	//thats all for here. head on over to the fragment shader. 
